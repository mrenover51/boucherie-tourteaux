...
export function Breadcrumbs({ items }: { items: readonly Crumb[] }) {
  return (
    <nav aria-label="Fil d’Ariane" className="border-b border-border-dark/10 bg-background">
        ...
        {items.map((item, index) => (
          <li key={item.href} className="flex items-center gap-2">
            ...
            {index === items.length - 1 ? (
              ...
              <Link href={item.href} className="transition hover:text-foreground">{item.name}</Link>
            )}
          </li>
        ))}
      </ol>

...
export function LocalLanding({ content }: { content: LocalLandingContent }) {
  ...
  return (
    <>
      <section className="relative isolate flex min-h-[68svh] items-end overflow-hidden bg-surface-dark text-on-dark">
        ...
        <Container className="relative z-10 pb-16 pt-40">
          <p className="text-xs font-bold uppercase tracking-[.22em] text-accent-light">{content.eyebrow}</p>
          <h1 className="mt-6 max-w-5xl font-serif text-[clamp(3.4rem,7vw,7rem)] leading-[.9] tracking-[-.04em]">{content.h1}</h1>
          <p className="mt-7 max-w-2xl text-base leading-8 text-on-dark/75">{content.intro}</p>
        </Container>
      </section>
      ...
      <section className="bg-background py-20 sm:py-28">
        <Container className="grid gap-14 lg:grid-cols-[1fr_.42fr] lg:gap-20">
          <div className="space-y-16">
            {content.sections.map((section) => (
              <section key={section.title}>
                <h2 className="font-serif text-4xl leading-tight text-foreground sm:text-5xl">{section.title}</h2>
          ...
          <aside className="h-fit border border-border-dark/10 bg-surface p-7 lg:sticky lg:top-32">
            <MapPin className="size-7 text-accent" />
            <h2 className="mt-6 font-serif text-3xl text-foreground">Venir à la boutique</h2>
            ...
            <a href={directions} target="_blank" rel="noreferrer" className="premium-button mt-7 w-full justify-center bg-surface-dark px-5 py-4 text-xs font-bold uppercase tracking-[.12em] text-on-dark">Itinéraire <ArrowRight className="size-4" /></a>
            <a href={siteConfig.phoneHref} className="mt-3 flex items-center justify-center gap-2 py-3 font-serif text-xl text-foreground"><Phone className="size-4" />{siteConfig.phone}</a>
            <div className="mt-6 border-t border-border-dark/10 pt-5 text-sm leading-7 text-foreground/60"><Clock3 className="mr-2 inline size-4 text-accent" />Ouvert du mardi au dimanche midi. Consultez les <Link href="/contact" className="text-foreground underline underline-offset-4">horaires détaillés</Link>.</div>
          </aside>
        </Container>
      </section>
      <section className="bg-surface py-16">
        <Container>
          <h2 className="font-serif text-4xl text-foreground">Préparer votre visite</h2>
          <p className="mt-5 max-w-3xl leading-8 text-foreground/60">Consultez les <Link href="/produits" className="text-foreground underline underline-offset-4">catégories proposées en boutique</Link>, découvrez le <Link href="/traiteur" className="text-foreground underline underline-offset-4">service traiteur à Avize</Link> ou appelez avant votre déplacement pour vérifier une disponibilité. La livraison et la vente à emporter sont proposées ; leurs modalités et la commune concernée doivent être confirmées directement avec la boucherie.</p>

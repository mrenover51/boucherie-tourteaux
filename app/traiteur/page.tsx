...
import { TraiteurGallery } from "@/components/traiteur/traiteur-gallery";
...
export default function TraiteurPage() {
  ...
  return <div>
    ...
    <TraiteurGallery />
    <section className="bg-background py-20 sm:py-28"><Container><p className="text-xs font-bold uppercase tracking-[.22em] text-foreground">Services confirmés</p><h2 className="mt-6 max-w-3xl font-serif text-5xl leading-[.95] text-foreground">Des modalités claires avant de vous déplacer.</h2><div className="mt-14 grid gap-4 md:grid-cols-3">{services.map(({icon:Icon,title,text})=><article key={title} className="min-h-72 rounded-xl border border-border-dark/10 bg-surface p-7"><Icon className="size-7 text-accent"/><h3 className="mt-16 font-serif text-3xl text-foreground">{title}</h3><p className="mt-5 text-sm leading-6 text-foreground/55">{text}</p></article>)}</div></Container></section>
